...
def scenario1_incline():
    ...
    if angle > 75:
        print(f"     Угол {angle}° очень крутой!")
    ...
    if any(results['is_slipping']):
        print(f"     Был переход в режим проскальзывания!")
    else:
...
def scenario2_slipping():
    ...
    print(f"\n🔮 Прогноз: {'  БУДЕТ проскальзывание' if will_slip else ' Качение без проскальзывания'}")
    ...
    if slipping_count > 0:
        print(f"     Проскальзывание: {slipping_percent:.1f}% времени")
...
def scenario5_multiple_balls():
    ...
    print(f"     Время симуляции: {results['time'][-1]:.2f} с")
...
def scenario6_custom():
    ...
    total_time = input_float("\n  Время симуляции (сек)", default=5.0, min_val=0.1)
...
def main():
    ...
    while True:
        ...
        print("  2. Переход в режим проскальзывания ")
